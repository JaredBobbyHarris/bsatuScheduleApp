...
def login( ):
    ...
    if form.validate_on_submit():
        ...
        if(len(userDict) == 1):
            ...
            error = 'Пароль или логин неверны! Попробуйте снова.'
...
def ForgotPassword():
   ...
   if form.validate_on_submit():
        ...
        if db.CheckEmail(email):
            message = "Для обновы пароля отправлено!"
            token =s.dumps(email, salt='email-confirm')
            msg = Message('Hello', sender = 'testboss@ukr.net', recipients = [email])
            Link = url_for('confirm_email', token=token, _external=True)
            msg.body = 'Ссылка для смены паролоида: {}'.format(Link)
            mail.send(msg)
        else:
        ...
        try:
            ...
            if icorpus and corpus and ncalss:
                ...
                db.AddCorpus(icorpus)
                 ...
                 message = "Недостаточно данных для записи!"
        except (Exception, psycopg2.DatabaseError) as error:
            message = "Ошибка " + str(error)
...
def UpdateStudens():
    ...
    if form.is_submitted():
        ...
        if len(r) == 2:
            ...
            vGroups       = db.GetGroups()
            db.Close()
            message = "Надо выбрать хотя бы одно поле!"
...
def UpdateGroups():
    ...
    if form.is_submitted():
         ...
         if len(r) == 2:
             ...
             vDepartment = db.GetDepartment()
             db.Close()
             message = "Надо заполнить хотя бы одно поле!"
...
def ShowInfoAmountSubjectNextDay():
    ...
    if request.method == "POST":
        ...
        message = "Количество дисциплин, которые будут проходить завтра в корпусе "+ readBuild + " равно: "+ str(vAmountSubject[0][0])
        db.Close()
